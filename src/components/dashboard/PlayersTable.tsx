                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex space-x-2">
                      {socialLinks.steam && (
                        <a href={socialLinks.steam} target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform" title="Profil Steam" onClick={e => e.stopPropagation()}> <svg
                          fill="currentColor"
                          viewBox="0 0 32 32"
                      ...
                      {socialLinks.discord && (
                        <a href={socialLinks.discord} target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform" title="Profil Discord" onClick={e => e.stopPropagation()}><svg
                          fill="currentColor"
                          viewBox="0 0 32 32"
                          className="w-6 h-6 hover:text-purple-600 transition-colors"
                          xmlns="http://www.w3.org/2000/svg"
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <button
                      ...
                      className={`p-1 rounded transition-colors ${isPlayerFavorite(player.name)
                    ...
                    <td colSpan={6} className="px-6 py-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-1 duration-200">
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold uppercase text-zinc-500 tracking-wider">Playtime & Session</h4>
                          {statsLoading ? (
                            <div className="h-4 bg-zinc-200 dark:bg-zinc-800 rounded w-24 animate-pulse"></div>
                          ) : (
                            <div className="text-sm">
                              {playerStats?.playtime?.[0]?.seconds ? (
                                <span>{(playerStats.playtime[0].seconds / 3600).toFixed(1)} hours on record</span>
                              ) : (
                                <span className="text-zinc-400 italic">No playtime data available</span>
                              )}
                            </div>
                          )}
                        </div>
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold uppercase text-zinc-500 tracking-wider">Identifiers</h4>
                          <div className="flex flex-wrap gap-1">
                            {player.identifiers?.map(id => (
                              <span key={id} className="text-[10px] px-1.5 py-0.5 bg-zinc-200 dark:bg-zinc-800 rounded font-mono truncate max-w-[200px]" title={id}>
                                {id}
                              </span>
                            )) || <span className="text-zinc-400 text-xs italic">Hidden identifiers</span>}
                          </div>
                        </div>
                      </div>
                    </td>
